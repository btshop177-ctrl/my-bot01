...
class UsersDatabase:
        ...
        if os.path.exists(DB_FILE):
            try:
                with open(DB_FILE, "r", encoding="utf-8") as f:
                    ...
                    data["settings"].setdefault("required_channels", [])
                    return data
    ...
    def get_required_channels(self, only_enabled=True):
        channels = self.data.get("settings", {}).get("required_channels", [])
        if only_enabled:
            return [c for c in channels if c.get("enabled", True)]
        return channels

    def add_required_channel(self, chat_id, title, kind="channel",
                             username="", invite_link=""):
        channels = self.data["settings"]["required_channels"]
        for c in channels:
            if c["chat_id"] == int(chat_id):
                c.update({
                    "title": title, "kind": kind, "enabled": True,
    ...
    def remove_required_channel(self, index):
        channels = self.data["settings"]["required_channels"]
    ...
    def toggle_required_channel(self, index):
        channels = self.data["settings"]["required_channels"]
        if 0 <= index < len(channels):
    ...
    def get_user_by_account(self, account_id):
        """یافتن کاربر بر اساس آیدی اکانتی که سشن آن ساخته شده"""
        for u in self.data["users"].values():
            if u.get("account_id") == int(account_id):
                return u
        return self.data["users"].get(str(account_id))
