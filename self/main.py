...
# ─── ربات پنل مشترک (ربات مدیریت) ───
# اگر کاربر توکن ربات اختصاصی نداشته باشد، پنل از طریق ربات مدیریت کار می‌کند
PANEL_BOT = _env("SELF_PANEL_BOT", default="") or None

# فایل دستورات پنل مشترک (کنار کانفیگ کاربر)
COMMANDS_FILE = os.path.join(
    os.path.dirname(os.path.abspath(CONFIG_FILE)), "commands.json"
)
...
# ═══════════════════════════════════════
# همگام‌سازی پنل مشترک (ربات مدیریت)
# ربات مدیریت روی config.json این کاربر تغییر می‌نویسد و
# دستورهای لحظه‌ای را در commands.json می‌گذارد؛ این وظیفه
# آن‌ها را هر ۲ ثانیه می‌خواند و در همان پروسه اعمال می‌کند.
# ═══════════════════════════════════════

async def panel_sync_watcher(config, clock_mgr, banner_mgr, action_mgr, spam_mgr):
    def _watched_snapshot():
        return (
            config.get("clock_enabled"),
            config.get("clock_bio_enabled"),
            config.get("clock_font"),
            config.get("tapchi_enabled"),
            config.get("banner_mode"),
            json.dumps(config.get("banners", {}), sort_keys=True,
                       ensure_ascii=False),
            json.dumps(config.get("actions_list", []), sort_keys=True,
                       ensure_ascii=False),
        )
    ...
    def _reload_config_from_disk():
        try:
            ...
            print(f"⚠️ watcher: خطا در خواندن کانفیگ: {str(e)[:60]}")

    def _run_panel_command(cmd):
        name = cmd.get("cmd", "")
        try:
            if name == "spam_stop_all":
                spam_mgr.stop_all()
                print("🎛 دستور پنل: توقف همه اسپم‌ها")
        except Exception as e:
            print(f"⚠️ خطا در دستور پنل {name}: {e}")

    def _consume_commands_file():
        if not os.path.exists(COMMANDS_FILE):
            return
        tmp = COMMANDS_FILE + ".run"
        try:
            os.replace(COMMANDS_FILE, tmp)
        except OSError:
            return
        try:
            ...
            try:
                ...
                pass
        for cmd in commands:
            _run_panel_command(cmd)

    def _apply_clock_state():
        if config.get("clock_enabled"):
            clock_mgr.start()
        else:
            clock_mgr.stop()

    def _apply_tapchi_state():
        if config.get("tapchi_enabled"):
            banner_mgr.start_all()
        else:
            banner_mgr.stop_all()

    def _apply_actions_state():
        action_mgr.stop_all_tasks()
        action_mgr.start_saved_actions()

    prev = _watched_snapshot()
    last_mtime = 0
    print("🎛 واچر همگام‌سازی پنل مشترک فعال شد")
    while True:
        ...
        try:
            ...
            _consume_commands_file()

            cur = _watched_snapshot()
            if cur != prev:
                # ساعت روشن/خاموش
                if cur[0] != prev[0]:
                    _apply_clock_state()
                # فونت ساعت → رفرش فوری متن
                if cur[2] != prev[2]:
                    clock_mgr.last_time = ""
                # تپچی روشن/خاموش یا تغییر لیست بنرها
                if cur[3] != prev[3] or cur[5] != prev[5]:
                    _apply_tapchi_state()
                # تغییر لیست اکشن‌ها
                if cur[6] != prev[6]:
                    _apply_actions_state()
                prev = cur

        except asyncio.CancelledError:
            break
        except Exception as e:
            print(f"⚠️ خطا در واچر پنل: {str(e)[:80]}")
...
async def main():
    ...
    if bot_client:
        ...
        print("ℹ️ BOT_TOKEN تنظیم نشده - پنل اینلاین از ربات مدیریت")
    ...
    # ─── ربات پنل: یا ربات اختصاصی کاربر یا ربات مدیریت مشترک ───
    panel_bot_username = bot_username or PANEL_BOT

    # ─── واچر همگام‌سازی با ربات مدیریت مشترک ───
    if PANEL_BOT:
        asyncio.ensure_future(
            panel_sync_watcher(config, clock, banner, action, spam)
        )
        print(f"🎛 پنل مشترک از طریق ربات مدیریت: @{PANEL_BOT}")
    ...
    async def panel_command(event):
        if not panel_bot_username:
            await event.edit(
                "❌ **ربات پنل فعال نیست**\n"
                "توکن ربات یا ربات مدیریت مشترک تنظیم نشده است."
            )
            return
        try:
            await event.delete()
            results = await user_client.inline_query(
                panel_bot_username, "panel"
            )
    ...
    async def commands_handler(event):
        ...
        # ═══════════════════════════════════════
        # آموزش نقش‌های بازی گرگینه
        # «آموزش نقش» یا «آموزش دردسر» یا «آموزش رویا»
        # ═══════════════════════════════════════
        elif text.startswith("آموزش ") and is_command(text):
            try:
                import werewolf
                response = werewolf.get_training_response(text)
                if response:
                    await event.edit(response)
            except Exception as e:
                await event.edit(f"❌ خطا در آموزش نقش: `{str(e)[:80]}`")
            return
